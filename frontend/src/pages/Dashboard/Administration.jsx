...
const Administration = () => {
  ...
  return (
    <div className="w-full h-screen pt-[85px] px-8 py-10">
      ...
      {selection === "address" && <div>Address</div>}
      {selection === "contact" && <div>Contact</div>}
  ...
  const onSubmit = async (data) => {
    try {
        const res = await axios.post(
          `${import.meta.env.VITE_API_URL}/administration/main-info/${id}` , data
        );
  ...
  return (
    <form className="flex flex-col gap-4 w-full" onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="flex items-center gap-2">
        ...
        <Input id="start-date" type="date" {...register('start_fin_year')} placeholder="Start Date" className="w-1/3" />
      </div>
